...
export default function MembersScreen() {
  ...
  const fetchMembers = useCallback(async (isRefresh = false) => {
    ...
    setError('')
    try {
      const { data } = await api.get('/members') // no search param sent – we filter locally
  ...
  const members = useMemo(() => {
    ...
    return allMembers.filter(
      (m) =>
        ...
        m.reg_no.toLowerCase().includes(s) ||
        m.phone.toLowerCase().includes(s) ||
        m.national_id.toLowerCase().includes(s) ||
        m.bank.toLowerCase().includes(s) ||
        m.account_no.toLowerCase().includes(s)
    )
  }, [allMembers, search])
    ...
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        <View style={styles.regBadge}>
          <Text style={styles.regText}>{item.reg_no}</Text>
        </View>
      ...
      <View style={styles.cardBody}>
        ...
        {item.national_id !== 'N/A' && (
          <View style={styles.detailRow}>
            <Ionicons name="card-outline" size={14} color="#6b5e53" />
            <Text style={styles.detailText}>{item.national_id}</Text>
          </View>
        )}
        {item.bank !== 'N/A' && (
          <View style={styles.detailRow}>
            <Ionicons name="business-outline" size={14} color="#6b5e53" />
            <Text style={styles.detailText}>{item.bank} {item.account_no !== 'N/A' ? `· ${item.account_no}` : ''}</Text>
          </View>
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  searchInputWrapper: {
    ...
    borderRadius: 12, borderWidth: 1, borderColor: '#d9d0c7', paddingHorizontal: 12,
  },
  ...
  regBadge: {
    backgroundColor: '#8c6239', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4, marginRight: 10,
  },
  regText: { color: '#fff', fontWeight: '700', fontSize: 12 },
  memberName: { fontSize: 16, fontWeight: '700', color: '#3d2b1f', flex: 1 },
  cardBody: { marginTop: 4 },
  detailRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
  detailText: { fontSize: 13, color: '#6b5e53', marginLeft: 6 },
