...
const getDayAbbrev = (date: Date): string => date.toLocaleDateString('en-US', { weekday: 'short' }).slice(0, 2);

// ---------- Subcomponents ----------
const StatCard = ({ label, value, onPress }: { label: string; value: number; onPress?: () => void }) => {
  console.log(`[UI] Rendering StatCard: ${label} = ${value}`);
  return (
    <TouchableOpacity style={styles.statCard} onPress={onPress} activeOpacity={0.7} disabled={!onPress}>
...
const SplitPill = ({ cherry, mbuni }: { cherry: number; mbuni: number }) => {
  console.log(`[UI] SplitPill: cherry=${cherry}, mbuni=${mbuni}`);
  return (
...
const TrendBar = ({ data }: { data: number[] }) => {
  console.log(`[UI] TrendBar data: ${data.join(', ')}`);
    ...
    <View style={styles.trendContainer}>
      {data.map((value, idx) => (
        <View key={idx} style={styles.trendBarWrapper}>
          ...
          <View style={[styles.trendBar, { height: Math.max(4, (value / max) * 40), backgroundColor: idx === 6 ? '#3d2b1f' : '#e0d9d0' }]} />
          <Text style={[styles.trendLabel, idx === 6 && styles.trendLabelToday]}>{getDayAbbrev(days[idx])}</Text>
        </View>
      ))}
    </View>
  );
...
const RecentActivityRow = ({ item, isAdmin }: { item: ActivityItem; isAdmin: boolean }) => {
  console.log(`[UI] RecentActivityRow: ${item.type}, ${item.memberName}, ${item.kgs}kg`);
  ...
  const typeLabel = isDelivery ? (item.coffeeType === 'CHERRY' ? 'Cherry' : 'Mbuni') : (item.coffeeType === 'CHERRY' ? 'Sold Cherry' : 'Sold Mbuni');
  const memberName = isDelivery ? item.memberName : `${item.memberName} → ${item.counterPartyName}`;
  return (
    <TouchableOpacity style={styles.activityRow} onPress={() => {
      if (isDelivery) navigation.navigate('DeliveriesTab', { screen: 'Deliveries', params: { highlightId: item.id } });
      else navigation.navigate('TransactionsTab', { screen: 'Transactions', params: { highlightId: item.id } });
    }}>
...
const ActionButton = ({ icon, title, onPress, accent }: { icon: any; title: string; onPress: () => void; accent?: boolean }) => (
  <TouchableOpacity style={[styles.actionCard, accent && styles.actionCardAccent]} onPress={onPress} activeOpacity={0.7}>
...
export default function DashboardScreen() {
  ...
  const fetchDashboard = useCallback(async (isRefresh = false) => {
    console.log('[Dashboard] fetchDashboard called, isRefresh:', isRefresh);
    const now = Date.now();
    if (isFetching.current) {
      console.log('[Dashboard] Already fetching, skipping');
      return;
    }
    if (!isRefresh && now - lastFetchTime.current < 2000) {
      console.log('[Dashboard] Cooldown active, skipping');
      return;
    }
    ...
    setError('');
    try {
      console.log('[Dashboard] Calling API /dashboard/mobile');
      const { data } = await api.get('/dashboard/mobile');
      console.log('[Dashboard] API response received:', JSON.stringify(data, null, 2));
      ...
      console.log('[Dashboard] fetchDashboard finished');
    }
  }, []);
  ...
  useEffect(() => {
    const onRecord = () => {
      console.log('[Dashboard] Event received: delivery/transaction/sync recorded, refreshing');
      fetchDashboard(true);
      refreshPendingCount();
    };
  ...
  console.log('[Dashboard] Rendering with data:', {
    ...
    todayUserDeliveries: data.today.user.deliveriesKg,
    todayUserTransactions: data.today.user.transactionsKg,
    todayFactoryDeliveries: data.today.factory.deliveriesKg,
    todayFactoryTransactions: data.today.factory.transactionsKg,
    trend: data.trend,
  ...
  console.log('[Dashboard] UI flags:', { showUserDeliverySplit, showFactoryDeliverySplit, showTrend, showTransactionBought });
    ...
    >
      ...
      <View style={styles.header}>
          ...
          {showSeasonPill && (
            <View style={styles.seasonPill}>
              ...
              <Text style={styles.seasonText}>
                {activeSeason.name}{daysRemaining !== null && `  ·  ${daysRemaining} day${daysRemaining !== 1 ? 's' : ''} left`}
              </Text>
            </View>
          )}
        ...
        <View style={styles.headerRight}>
          <Text style={styles.updatedText}>Updated {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
        </View>
      </View>

      {/* Pending Sync Banner */}
      {pendingCount > 0 && (
        <TouchableOpacity style={styles.pendingBanner} onPress={() => navigation.navigate('PendingDeliveries')}>
...
<View style={styles.section}>
  ...
  <View style={styles.statsRow}>
    {/* Use deliveriesCount (number of deliveries) instead of deliveriesKg */}
    <StatCard 
      label="My Deliveries" 
      value={todayData.user.deliveriesCount} 
      onPress={() => navigation.navigate('DeliveriesTab')} 
    />
    {/* Use transactionsCount (number of transactions) instead of transactionsKg */}
    <StatCard 
      label="My Transactions" 
      value={todayData.user.transactionsCount} 
    ...
    />
      ...
      {isAdmin && (
        <View style={styles.section}>
          ...
          <View style={styles.statsRow}>
            <StatCard label="Members" value={data.memberCount || 0} onPress={() => navigation.navigate('Members')} />
            <StatCard label="Workers" value={data.workerCount || 0} onPress={() => navigation.navigate('Workers')} />
          </View>
        </View>
      )}
      ...
      {activeSeason && (
        <View style={styles.seasonCard}>
          ...
          <View style={styles.seasonRow}>
            <View style={styles.seasonItem}><View style={[styles.seasonDot, { backgroundColor: '#3d2b1f' }]} /><Text style={styles.seasonLabel}>Cherry</Text><Text style={styles.seasonValue}>{formatNumber(seasonProgress.deliveries.cherry)} kg</Text></View>
            <View style={styles.seasonItem}><View style={[styles.seasonDot, { backgroundColor: '#8c6239' }]} /><Text style={styles.seasonLabel}>Mbuni</Text><Text style={styles.seasonValue}>{formatNumber(seasonProgress.deliveries.mbuni)} kg</Text></View>
            <View style={styles.seasonItem}><View style={[styles.seasonDot, { backgroundColor: '#1a1512' }]} /><Text style={styles.seasonLabel}>Total</Text><Text style={styles.seasonValue}>{formatNumber(seasonProgress.deliveries.total)} kg</Text></View>
          </View>
          <View style={styles.seasonDivider} />
          <View style={styles.seasonRow}>
            <View style={styles.seasonItem}><View style={[styles.seasonDot, { backgroundColor: '#1565c0' }]} /><Text style={styles.seasonLabel}>Transacted</Text><Text style={styles.seasonValue}>{formatNumber(seasonProgress.transactions.sold)} kg</Text></View>
            {showTransactionBought && (
              <View style={styles.seasonItem}><View style={[styles.seasonDot, { backgroundColor: '#2e7d32' }]} /><Text style={styles.seasonLabel}>Bought</Text><Text style={styles.seasonValue}>{formatNumber(seasonProgress.transactions.bought)} kg</Text></View>
            )}
            <View style={styles.seasonItem}><View style={[styles.seasonDot, { backgroundColor: '#1a1512' }]} /><Text style={styles.seasonLabel}>Total</Text><Text style={styles.seasonValue}>{formatNumber(seasonProgress.transactions.total)} kg</Text></View>
          </View>
        </View>
      )}
      ...
      <View style={styles.section}>
        ...
        {recentActivity.length === 0 ? (
          ...
          recentActivity.map(item => <RecentActivityRow key={item.id} item={item} isAdmin={isAdmin} />)
        )}
      </View>
      ...
      <View style={styles.actionsSection}>
        ...
        <View style={styles.actionsGrid}>
          ...
          {isAdmin && <ActionButton icon="person-add" title="Add Member" onPress={() => navigation.navigate('AddMember')} />}
          {pendingCount > 0 && <ActionButton icon="cloud-upload" title={`Sync Now (${pendingCount})`} onPress={() => navigation.navigate('PendingDeliveries')} accent />}
        </View>
      </View>
    </ScrollView>
  );
